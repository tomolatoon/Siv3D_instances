...
export namespace tomolatoon
{
	/// @brief 新しい Iframe/シザー矩形 領域が現在の Iframe/シザー矩形 領域と交差していない場合に
	///        例外を投げるかどうかを設定するフラグ
	using ForceNextIntersectCurrent = YesNo<struct ForceNextIntersectCurrentTag>;

	/// @brief 新しい Iframe/シザー矩形 領域が現在の Iframe/シザー矩形 領域からはみ出している場合に
	///        はみ出した部分を切り取るかどうかを設定するフラグ
	...
	private:
		...
		static Rect NewIframeRect(const Rect request, const bool isCropped, const bool isForceNextIntersectCurrent)
		{
			...
			if (isForceNextIntersectCurrent)
			{
				...
				{
					throw Error{U"Error: ScopedIframe2D: request is not intersects with current viewport"};
				}
			}
	...
	public:
		...
		ScopedIframe2D(
			...
			: s3d::ScopedViewport2D(NewIframeRect(Iframe::ToGlobal(local), isCropped.getBool(), isForceNextIntersectCurrent.getBool()))
		...
		ScopedIframe2D(
			...
			EnableCroppingNextWithCurrent isCropped                   = EnableCroppingNextWithCurrent::No,
			ForceNextIntersectCurrent     isForceNextIntersectCurrent = ForceNextIntersectCurrent::No)
			: s3d::ScopedViewport2D(NewIframeRect(Iframe::ToGlobal(local.value()), isCropped.getBool(), isForceNextIntersectCurrent.getBool()))
		...
		ScopedIframe2D(
			...
			EnableCroppingNextWithCurrent isCropped                   = EnableCroppingNextWithCurrent::No,
			ForceNextIntersectCurrent     isForceNextIntersectCurrent = ForceNextIntersectCurrent::No)
			: s3d::ScopedViewport2D(NewIframeRect(global.value(), isCropped.getBool(), isForceNextIntersectCurrent.getBool()))
			, s3d::Transformer2D(Mat3x2::Identity(), Mat3x2::Translate(Iframe::ToLocal(global.value()).tl()))
		{}
	};
	...
	struct ScopedScissorRect2D
		: s3d::ScopedRenderStates2D
	{
	private:
		// @note 入出力する Rect は全て Global(Scene) 座標系であることに注意
		static ScopedRenderStates2D NewScissorRect(const Rect request, const bool isCropped, const bool isForceNextIntersectCurrent)
		{
			const Rect scissor = []() {
			...
			if (isForceNextIntersectCurrent)
			{
				...
				{
					throw Error{U"Error: ScopedScissorRect2D: request is not intersects with current viewport"};
				}
			}
	...
	public:
		...
		ScopedScissorRect2D(Rect                          rect,
		                    PositionBased                 base                        = PositionBased::Local,
		                    EnableCroppingNextWithCurrent isCropped                   = EnableCroppingNextWithCurrent::No,
		                    ForceNextIntersectCurrent     isForceNextIntersectCurrent = ForceNextIntersectCurrent::No)
			: s3d::ScopedRenderStates2D(NewScissorRect(base == PositionBased::Local ? Iframe::ToGlobal(rect) : rect, isCropped.getBool(), isForceNextIntersectCurrent.getBool()))
		{}

		/// @brief シザー矩形を指定して ScopedRenderStates2D を作成する（Local(Iframe) 座標系）
		/// @param local 新しいシザー矩形の領域（Local(Iframe) 座標系）
		ScopedScissorRect2D(Arg::local_<Rect>             local,
		                    EnableCroppingNextWithCurrent isCropped                   = EnableCroppingNextWithCurrent::No,
		                    ForceNextIntersectCurrent     isForceNextIntersectCurrent = ForceNextIntersectCurrent::No)
			: s3d::ScopedRenderStates2D(NewScissorRect(Iframe::ToGlobal(local.value()), isCropped.getBool(), isForceNextIntersectCurrent.getBool()))
		{}

		/// @brief シザー矩形を指定して ScopedRenderStates2D を作成する（Global(Scene) 座標系）
		/// @param global 新しいシザー矩形の領域（Global(Scene) 座標系）
		ScopedScissorRect2D(Arg::global_<Rect>            global,
		                    EnableCroppingNextWithCurrent isCropped                   = EnableCroppingNextWithCurrent::No,
		                    ForceNextIntersectCurrent     isForceNextIntersectCurrent = ForceNextIntersectCurrent::No)
			: s3d::ScopedRenderStates2D(NewScissorRect(global.value(), isCropped.getBool(), isForceNextIntersectCurrent.getBool()))
		{}
	};
} // namespace tomolatoon
