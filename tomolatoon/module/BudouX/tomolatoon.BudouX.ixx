...
export namespace tomolatoon
{
	...
	struct BudouXBreakView : std::ranges::view_interface<BudouXBreakView<View>>
	{
		...
		struct iterator
		{
			...
			iterator& operator++()
			{
				...
				if (isEnd())
				{
					throw Error{U"[BudouXBreakView::Iterator::operator++]: cannnot advance over end."};
				}
		...
		BudouXBreakView(View view, std::reference_wrapper<BudouXParser> parser)
			: m_view{std::move(view)}
			, m_parser{std::ref(std::as_const(parser.get()))}
		{}
	...
	static_assert(std::input_or_output_iterator<BudouXBreakView<std::views::all_t<String>>::iterator<true>>);
	static_assert(std::input_or_output_iterator<BudouXBreakView<std::views::all_t<String>>::iterator<false>>);
	...
	static_assert(std::forward_iterator<BudouXBreakView<std::views::all_t<String>>::iterator<false>>);
	static_assert(std::forward_iterator<BudouXBreakView<std::views::all_t<String>>::iterator<true>>);
