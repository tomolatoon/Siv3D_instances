...
export namespace tomolatoon
{
	...
	struct LerpTransition
	{
		...
		constexpr LerpTransition(const Duration& inDuration, const Duration& outDuration, double start, double finish, Arg::ratio_<double> init)
			: m_transition(inDuration, outDuration, init.value())
			, m_range(start, finish)
		{}

		constexpr LerpTransition(const Duration& inDuration, const Duration& outDuration, double start, double finish, Arg::value_<double> init)
		...
		{
			return Math::Lerp(m_range.start, m_range.finish, transition().easeInOut(easingFunction));
		}
